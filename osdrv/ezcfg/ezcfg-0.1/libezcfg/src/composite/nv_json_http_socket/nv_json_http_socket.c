 ...
 * File Name    : nv_json_http_socket/nv_json_http_socket.c
 *
  ...
  struct ezcfg_http *http)
{
  ...
  if (ezcfg_common_set_nvram_entries(ezcfg, list) == EZCFG_RET_OK) {
 ...
 * Public functions
 **/
int ezcfg_nv_json_http_socket_delete(struct ezcfg_nv_json_http_socket *njhs)
{
  struct ezcfg *ezcfg;
  ...
  if (njhs->json != NULL)
    ezcfg_json_delete(njhs->json);

  if (njh->http != NULL)
    ezcfg_http_delete(njhs->http);
 ...
 **/
struct ezcfg_nv_json_http_socket *ezcfg_nv_json_http_socket_new(struct ezcfg *ezcfg, char *ns)
{
  struct ezcfg_nv_json_http_socket *njhs;

  ASSERT(ezcfg != NULL);

  /* initialize nvram json http binding info builder data structure */
  njhs = calloc(1, sizeof(struct ezcfg_nv_json_http_socket));
  if (njh == NULL) {
