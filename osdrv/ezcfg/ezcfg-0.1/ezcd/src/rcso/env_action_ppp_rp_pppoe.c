 ...
 * Module Name  : env_action_ppp_rp_pppoe.c
 *
 * Description  : ezbox env agent runs rp-pppoe client service
 *
 * Copyright (C) 2008-2013 by ezbox-project
 *
 * History      Rev       Description
 * 2012-09-19   0.1       Write it from scratch
...
#else
int env_action_ppp_rp_pppoe(int argc, char **argv)
#endif
{
	int rc;
	char buf[128];
	...
	if (strcmp(argv[1], "lan") == 0 &&
            utils_service_binding_lan(NVRAM_SERVICE_OPTION(RC, PPP_RP_PPPOE_BINDING)) == true) {
		/* It's good */
	}
	else
#endif
	...
	if (strcmp(argv[1], "wan") == 0 &&
            utils_service_binding_wan(NVRAM_SERVICE_OPTION(RC, PPP_RP_PPPOE_BINDING)) == true) {
		/* It's good */
	}
	else
#endif
	...
	case RC_ACT_RESTART :
	case RC_ACT_STOP :
		utils_system("start-stop-daemon -K -n pppd");
		if (flag == RC_ACT_STOP) {
			ret = EXIT_SUCCESS;
			break;
		}
	...
	case RC_ACT_START :
		rc = utils_nvram_cmp(NVRAM_SERVICE_OPTION(RC, PPP_RP_PPPOE_ENABLE), "1");
		if (rc < 0) {
			return (EXIT_FAILURE);
		}
