 ...
 * Module Name  : basic/binary_tree/binary_tree.c
 *
...
int ezcfg_binary_tree_node_del(struct ezcfg_binary_tree *tree, struct ezcfg_binary_tree_node *np)
{
  ...
  if (tree->data_del_handler) {
    tree->data_del_handler(np->data);
  }
  free(np);
  return EZCFG_RET_OK;
}
...
int ezcfg_binary_tree_reset(struct ezcfg_binary_tree *tree)
{
  ...
  if ((np->left == NULL) &&
      (np->right == NULL)) {
    ezcfg_binary_tree_node_del(tree, np);
    return EZCFG_RET_OK;
  }
