 ...
 * File Name    : agent/agent_master_load_socket_conf.c
 *
...
/* don't remove ctrl, nvram socket */
void ezcfg_agent_master_load_socket_conf(struct ezcfg_agent_master *master)
{
	...
	for (i = 0; i < socket_number; i++) {
		...
		if (sp == NULL) {
	    	...
	    	psp = ezcfg_agent_master_get_p_listening_sockets(master);
		...
		if (ezcfg_socket_enable_listening(sp, ezcfg_agent_master_get_sq_len(master)) < 0) {
